...
public class DialActivity extends Activity implements View.OnClickListener {
	...
	private void checkEnterPassword() {
		if (TextUtils.isEmpty(SettingManager.getInstance()
			...
			final EditText passwdCheck = (EditText) v
					...
					.setPositiveButton(R.string.enter_btn_ok,
							new DialogInterface.OnClickListener() {
								...
								public void onClick(
									...
									String passwdStr = passwd.getEditableText()
											.toString();
									String passwdCheckStr = passwdCheck
											.getEditableText().toString();
									if (TextUtils.isEmpty(passwdStr)
											|| TextUtils
													.isEmpty(passwdCheckStr)) {
										Toasts.getInstance(DialActivity.this)
												.show(R.string.enter_tips_password_empty,
														Toast.LENGTH_LONG);
										checkEnterPassword();
										return;
									}
									if (!passwdStr.equals(passwdCheckStr)) {
										Toasts.getInstance(DialActivity.this)
												.show(R.string.enter_tips_password_not_same,
														Toast.LENGTH_LONG);
										checkEnterPassword();
										return;
									}
									...
									if (!isNumeric(passwdStr)) {
										Toasts.getInstance(DialActivity.this)
												.show(R.string.enter_tips_password_number_only,
														Toast.LENGTH_LONG);
										checkEnterPassword();
										return;
									}
									...
									if (passwdStr.length() < 6) {
										Toasts.getInstance(DialActivity.this)
												.show(R.string.enter_tips_password_too_short,
														Toast.LENGTH_LONG);
										checkEnterPassword();
										return;
									}
									...
									String md5 = StringUtils
											.MD5Encode(passwdCheckStr);
									if (!TextUtils.isEmpty(md5)) {
										SettingManager.getInstance()
												.setKeyEnterPassword(md5);
									}
								}
							})
	...
	public void onClick(View view) {
		...
		if (!TextUtils.isEmpty(mNumberStr)) {
			...
			if (check
					.equals(SettingManager.getInstance().getKeyEnterPassword())) {
