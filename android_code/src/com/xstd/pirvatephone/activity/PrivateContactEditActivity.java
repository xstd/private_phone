			...
			public void onClick(View v) {
				...
				if (TextUtils.isEmpty(phone)) {
					...
					if(address.equals(phone)){
						...
						contactDatabase.delete(ContactInfoDao.TABLENAME, ContactInfoDao.Properties.Phone_number.columnName+"=?", new String[]{phone});
						contactInfoDao.insert(contactInfo);
						Toast.makeText(getApplicationContext(), "想联系人数据库中添加了一条新数据",
								Toast.LENGTH_SHORT).show();
						finish();
					}else{
						Cursor query = contactDatabase.query(ContactInfoDao.TABLENAME, null, ContactInfoDao.Properties.Phone_number.columnName+"=?", new String[]{phone}, null, null, null);
						if(query!=null && query.getCount()>0){
							//已存在,
							Toast.makeText(PrivateContactEditActivity.this, "该号码已经存在", Toast.LENGTH_SHORT).show();
						}else{
							//删除以前的---需要删除当前情景模式中的该号码
							contactDatabase.delete(ContactInfoDao.TABLENAME, ContactInfoDao.Properties.Phone_number.columnName+"=?", new String[]{address});
							ContextModelUtils.deleteModelDetail(PrivateContactEditActivity.this, new String[]{address});
