...
public class ShowSDCardFilesActivity extends BaseActivity implements
	...
	public void checkIsSelectAll() {
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			if (!fileInfo.isChecked) {
				Log.w(TAG, "变为全选");
				isSelectAll = false;
				btn_add.setText(R.string.private_comm_select_all);
				return;
			}
		}
		isSelectAll = true;
		btn_add.setText(R.string.rs_cancle_all);
	}

	private void initViews() {
		return_btn = (ImageButton) findViewById(R.id.ll_return_btn);
	...
	private void missingFile() {
		...
		if (willMoves.size() == 0) {
			Toasts.getInstance(this).show(R.string.rs_empty_select, 1);
			return;
		}
