...
//	private String[] contentTypes;
	...
	public void writeTo(OutputStream outstream) throws IOException {
        if (InternetConfig.DEBUG) {
            UtilsConfig.LOGD("<<<<<<<<<<<<<<< Begin write Multi Part body >>>>>>>>>>>>>>>");
        }
		...
		try {
	        ...
	        if (InternetConfig.DEBUG) {
	            ...
	            UtilsConfig.LOGD("     After write the Begin Data : " + beginStr);
	        }
			int index = 0;
			for (byte[] fileItemData : fileItemDatas) {
				...
				if (InternetConfig.DEBUG) {
				    ...
				    UtilsConfig.LOGD("     After write the file item data : " + fileItemStr);
	            }
				...
				if (file != null) {
				    if (InternetConfig.DEBUG) {
	                    String fileItemStr = new String(fileItemData);
	                    UtilsConfig.LOGD("    ((((( begin write the file : " + file.getAbsolutePath() + " ))))))");
	                }
					...
					while ((len = fis.read(buf)) != -1) {
					    if (InternetConfig.DEBUG) {
					        UtilsConfig.LOGD("     >>>>>> Before write part data of the file : " + file.getName()
						...
						if (InternetConfig.DEBUG) {
						    ...
						    UtilsConfig.LOGD("     >>>>>> After write part data of the file : " + file.getName()
			...
			if (InternetConfig.DEBUG) {
			    ...
			    UtilsConfig.LOGD("     After write the end data : " + endStr);
			    UtilsConfig.LOGD("<<<<<<<<<<<<<<< end write Multi Part body >>>>>>>>>>>>>>>");
            }
		} catch (Exception e) {
		    e.printStackTrace();  
		} finally {
