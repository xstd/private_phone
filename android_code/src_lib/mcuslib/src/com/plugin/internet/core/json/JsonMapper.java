...
public class JsonMapper {
     ...
     * 			value type
     * @return
     * 			bean description, may be NULL when class does not have a constructor annotated with JsonCreator
     */
...
//    	@SuppressWarnings("unchecked")
    	...
    	ConstructorParamDescription[] cpdArray = getDefaultConstructorDescription(constructor);
        		...
        		if (fieldAnnotations != null) {
            		for (Annotation annotation : fieldAnnotations) {
            			if (annotation.annotationType() == JsonProperty.class) {
            				...
            				FieldDescription fd = new FieldDescription(key, type, null, null, genericTypes[index]);
     ...
     * 			value type
     * @return
     * 			bean description, may be NULL when class does not have any field annotated with JsonProperty
     */
    	...
    	for (Field field : fields) {
    		if (field != null) {
    			if (field.isAnnotationPresent(JsonProperty.class)) {
    				...
    				FieldDescription fd = new FieldDescription(key, type, null, name, field.getGenericType());
    				fd.field = field;
    				beanDescription.addFieldDescription(fd);
    			}
    		}
...
//    		@SuppressWarnings("unchecked")
        	...
        	ConstructorParamDescription[] cpdArray = getDefaultConstructorDescription(beanDescription.constructor);
        	beanDescription.addConstructorParamDescription(cpdArray);
    	}
	...
	private void processMapping (BeanDescription beanDescription, JSONObject jsonObject) {
    	...
    	for (int i = 0 ; i < beanDescription.fieldDescriptions.size() ; i ++) {
    		FieldDescription fieldDescription = beanDescription.fieldDescriptions.get(i);
    ...
    private Object processPrimitiveArrayItem (JSONArray array, int index, BeanDescription nestBean) {
    	...
    	if (nestBean.fieldDescriptions != null && nestBean.fieldDescriptions.size() == 1) {
        	FieldDescription fd = nestBean.fieldDescriptions.get(0);
        	Class<?> type = fd.type;
     ...
     * Fill Java Bean with bean description
     * @param beanDescription
     * 			bean description that used to create java bean
     * @return
     * 			filled instance
     */
    private Object fillInstance (BeanDescription beanDescription) {
    	...
    	ArrayList<ConstructorParamDescription> cpds = beanDescription.constructorParamDescriptions;
    	...
    	if (constructor != null) {
			...
			if (cpds != null) {
    			...
    			int index = 0;
    			for (ConstructorParamDescription cpd : cpds) {
    		...
    		if (!useJsonCreator) {
    			...
    			ArrayList<FieldDescription> fds = beanDescription.fieldDescriptions;
    			if (fds != null) {
    				for (FieldDescription fd : fds) {
     ...
     * 			instance
     */
    private void fillProperty (FieldDescription fieldDescription, Object instance) {
     ...
     */
    private ConstructorParamDescription[] getDefaultConstructorDescription (Constructor<?> constructor) {

    	ConstructorParamDescription[] paramDescription = null;
    	if (constructor != null) {
    		Class<?>[] paramTypes = constructor.getParameterTypes();
    		if (paramTypes == null) {
    			return null;
    		}
    		paramDescription = new ConstructorParamDescription[paramTypes.length];
    		int index = 0;
    		for (Class<?> paramType : paramTypes) {
    			Object param = getDefaultValue(paramType);
    			ConstructorParamDescription temp = new ConstructorParamDescription(false, paramType, param);
