...
public class GestureImageViewTouchListener implements OnTouchListener {
        ...
        if (!inZoom) {
            ...
            if (!tapDetector.onTouchEvent(event)) {
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    if (DEBUG) {
                        Log.d(TAG, "[[OnTouch]] point count = " + event.getPointerCount());
                    }
                    ...
                    if (event.getPointerCount() > 1) {
                        ...
                        if (!touched) {
                            ...
                            if (isForCrop && handleDragInTouch(event.getX(), event.getY())) {
                                image.redraw();
                            } else if (!isForCrop && handleDrag(event.getX(), event.getY())) {
                                image.redraw();
                            } else {
                                
                                if (DEBUG) {
                                    Log.d(TAG, "[[OnTouch]] Enable ViewPager Filing");
                                }
                                image.getParent().getParent().requestDisallowInterceptTouchEvent(false);
                            }
    ...
    protected void handleUp() {
        ...
        if (isForCrop && currentScale < startingScale) {
            reset();
        } else {
    ...
    protected boolean handleDragInTouch(float x, float y) {
        current.x = x;
        current.y = y;
        ...
        float diffY = (current.y - last.y);
        if (DEBUG) {
            Log.d(TAG, "[[handleDragInTouch]] x = " + x + " y = " + y
        ...
        if (diffX != 0 || diffY != 0) {
            ...
            diff = (float) (effectiveHeight - displayHeight) / 2.0f;
    ...
    public void setDragBound(int top, int bottom) {
        cropDragTop = top;
        cropDragBottom = bottom;
    }
    ...
    protected void boundCoordinates() {
        ...
        if (isForCrop) {
            if (next.y < boundaryTop - cropDragBottom) {
                next.y = boundaryTop - cropDragBottom;
            } else if (next.y > boundaryBottom + cropDragTop) {
                next.y = boundaryBottom + cropDragTop;
            }
        } else {
